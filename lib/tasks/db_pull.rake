namespace :db do
  desc 'Pull production db to development'
  task :pull => [:dump, :restore]

  task :dump do
    dumpfile = "#{Rails.root}/tmp/latest.sql"
    puts 'PG_DUMP on production database...'
    production = Rails.application.config.database_configuration['production']
    system "ssh deploy@recr.ittalent.ee 'pg_dump -U deploy -d #{production['database']} -h #{production['host']} -F t' > #{dumpfile}"
    puts 'Done!'
  end

  task :restore do
    dev = Rails.application.config.database_configuration['development']
    dumpfile = "#{Rails.root}/tmp/latest.sql"
    puts 'PG_RESTORE on development database...'
    system "pg_restore --verbose --clean --no-acl --no-owner -h 127.0.0.1 -d #{dev['database']} #{dumpfile}"
    puts "pg_restore --verbose --clean --no-acl --no-owner -h 127.0.0.1 -d #{dev['database']} #{dumpfile}"
    puts 'Done!'
  end
end

